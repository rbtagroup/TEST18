...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const IAC_RIDES_KEY = "IAC_RIDES_V2";
  function todayKey(){
    const d = new Date();
    return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
  }
  function normalizeIacRides(arr){
    return (Array.isArray(arr) ? arr : []).map(Number).filter(n => Number.isFinite(n) && n > 0);
  }
  function loadIacRides(){
    try{
      const stored = JSON.parse(localStorage.getItem(IAC_RIDES_KEY) || "[]");
      if (Array.isArray(stored)) return normalizeIacRides(stored);
      if (stored && stored.date === todayKey()) return normalizeIacRides(stored.rides);
    }catch(_e){}
    return [];
  }
  function saveIacRides(arr){
    try{ localStorage.setItem(IAC_RIDES_KEY, JSON.stringify({ date: todayKey(), rides: normalizeIacRides(arr) })); }catch(_e){}
  }
...
// kOdevzdani set after vyplata
      ...
      const html = `
        ...
        <div class="hr"></div>
        <div class="row"><div class="key">Najeté km (auto):</div><div class="val">${kmReal}</div></div>
